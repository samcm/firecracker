...
impl Display for Resource {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Resource::Memlock => write!(f, "locked memory"),
        }
    }
}
...
impl ResourceLimits {
    ...
    fn set_limit(resource: Resource, target: libc::rlim_t) -> Result<(), JailerError> {
        ...
        // SAFETY: `resource` is a known-valid constant, and `&rlim` is non-dangling.
...
mod tests {
    ...
    fn test_display_resource() {
        ...
        assert_eq!(Resource::Memlock.to_string(), "locked memory".to_string());
    }
